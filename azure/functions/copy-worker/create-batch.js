// create-batch.js
// Usage: node create-batch.js [limit] [suffix]
// Produces batch.json with up to [limit] mp4 items resolved from TEST_SOURCE_URL.
...
async function main() {
  ...
  try {
    ...
    const out = path.join(base, 'batch.json');
    fs.writeFileSync(out, JSON.stringify(batchPayload, null, 2));
    console.log('Wrote batch.json with', items.length, 'items to', out);
  } catch (err) {
    console.error('Error creating batch.json:', err && err.message ? err.message : err);
    process.exit(1);
  }
}
