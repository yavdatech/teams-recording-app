// run-local-batch.js
// Usage: node run-local-batch.js [limit]
...
async function main() {
  ...
  // Prefer AZURE_ names (local.settings.json should provide them)
  ...
  try {
    ...
    const items = selected.map(c => ({ sourceDriveId: resolved.driveId, itemId: c.id, newName: c.name }));
    const batchPayload = { items, targetDriveId: process.env.TARGET_DRIVE_ID, targetFolderId: process.env.TARGET_FOLDER_ID };

    console.log('Invoking copy-worker with batch...');
    // create a minimal context with log and log.error
    const context = {
    ...
    console.error('Error running local batch:', err && err.message ? err.message : err);
